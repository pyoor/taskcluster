...
| get_worker_pool_with_capacity | read | worker_pool_id_in text | table(worker_pool_id text, provider_id text, previous_provider_ids jsonb, description text, config jsonb, created timestamptz, last_modified timestamptz, owner text, email_on_error boolean, provider_data jsonb, existing_capacity integer) | Get an existig worker pool.  The returned table will have one or (if no such worker pool is defined) zero rows. |
| get_worker_pools_with_capacity | read | page_size_in integer, page_offset_in integer | table(worker_pool_id text, provider_id text, previous_provider_ids jsonb, description text, config jsonb, created timestamptz, last_modified timestamptz, owner text, email_on_error boolean, provider_data jsonb, existing_capacity integer) | Get existing worker pools, ordered by `worker_pool_id`.  If the pagination arguments are both NULL, all rows are returned.<br />Otherwise, page_size rows are returned at offset page_offset. |
