...
suite(testing.suiteName(), function() {
  ...
  const update_worker_pool = async (db, wp = {}) => {
    ...
    for (const wp of with_cap) {
      assert(wp.existing_capacity !== undefined);
      delete wp.existing_capacity;
    }
  ...
  suite('existing capacity', function() {
    helper.dbTest('no workers', async function(db, isFake) {
      await create_worker_pool(db);
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 0);
    });
    helper.dbTest('single worker, capacity 1', async function(db, isFake) {
      await create_worker_pool(db);
      await create_worker(db, {capacity: 1, state: 'running'});
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 1);
    });
    helper.dbTest('single worker, capacity > 1', async function(db, isFake) {
      await create_worker_pool(db);
      await create_worker(db, {capacity: 64, state: 'running'});
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 64);
    });
    helper.dbTest('multiple workers, capacity 1', async function(db, isFake) {
      ...
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 4);
    });
    helper.dbTest('multiple workers, capacity > 1', async function(db, isFake) {
      await create_worker_pool(db);
      ...
      await create_worker(db, {worker_id: 'foo4', capacity: 1, state: 'running'});
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 161);
    });
    helper.dbTest('multiple workers, multiple states', async function(db, isFake) {
      ...
      assert.equal((await db.fns.get_worker_pool_with_capacity('wp/id'))[0].existing_capacity, 97);
    });
  });
});
